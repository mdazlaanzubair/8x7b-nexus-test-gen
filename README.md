...
8x7b-nexus-test-gen/
├── dataset/
│   ├── raw_data/           # Test cases excel sheet and Open AI chats generation
...
- **On Windows:**
    ...
    ```bash
    pip install langchain langchain-groq pandas numpy matplotlib seaborn scipy
    ```
...
- **On macOS/Linux:**
    ...
    ```bash
    pip3 install langchain langchain-groq pandas numpy matplotlib seaborn scipy
    ```
...
* **`raw_data/`**
    ...
    - **`test_cases.xlsx`**: Contains excel sheet containing test case in a tabulated structure generated by Human QA Engineers, GPT 4o, and o1 AI models.

    - **`chat_data`**: Contains chats data from which the models are used to generate test case of each software.
...
* **`evaluation_data/`**
    ...
    - **`remaining.json`**: Contains test cases that are left from evaluation processing if the program stuck, crashes or failed.
